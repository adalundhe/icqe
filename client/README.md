...
Open [http://'+process.env.REACT_APP_CALLBACK+'](http://'+process.env.REACT_APP_CALLBACK+') to view it in the browser.
...
```json
{
  ...
  "configurations": [{
    ...
    "type": "chrome",
    "request": "launch",
    "url": "http://'+process.env.REACT_APP_CALLBACK+'",
...
In the WebStorm menu `Run` select `Edit Configurations...`. Then click `+` and select `JavaScript Debug`. Paste `http://'+process.env.REACT_APP_CALLBACK+'` into the URL field and save the configuration.
...
```
Fetch API cannot load http://localhost:4000/api/todos. No 'Access-Control-Allow-Origin' header is present on the requested resource. Origin 'http://'+process.env.REACT_APP_CALLBACK+'' is therefore not allowed access. If an opaque response serves your needs, set the request's mode to 'no-cors' to fetch the resource with CORS disabled.
```
